...
func _draw() -> void:
	...
	if is_danger:
		...
		col = Color.WHITE if flash == 0 else Color(0.3, 0.3, 0.3, 1.0)
	...
	draw_string(THEME_FONT, Vector2(sz.x - offset - 180, sz.y - offset - 8), telemetry, HORIZONTAL_ALIGNMENT_RIGHT, 180, 10, Color(0.7, 0.7, 0.7, 1.0))
